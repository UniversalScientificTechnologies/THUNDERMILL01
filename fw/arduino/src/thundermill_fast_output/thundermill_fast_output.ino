// Thundermill with old USTTHUNDERMILLPCB01A with 3f motor
...
#define RANGE 100   // size of output buffer
  ...
  while(true)
  {
    ...
    for(uint8_t j=0; j<RANGE; j++)
    {
      ...
      for(uint8_t i=0; i<32; i++)
      {
        ...
        if (poll==0) if (!digitalRead(EXTINT)) poll=1;
        if (poll==1) if (digitalRead(EXTINT)) poll=2;
        if (poll==2) if (!digitalRead(EXTINT)) poll=3;
        if (poll==3) if (digitalRead(EXTINT)) poll=4;
      };
      if (poll==4) break;
      Serial.print(filtered / 32);     
      Serial.print(",");
    };
